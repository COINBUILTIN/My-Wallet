...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
    if ([textField isEqual:self.textboxToday])
    {
        ...
        if (self.view.frame.size.height<568) {
                self.datePicker.transform=CGAffineTransformMakeTranslation(0, -20);
        }
...
-(void) prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([segue.identifier isEqual:@"addTransfer"]){
        ...
        self.myPopover =[segue destinationViewController];
...
-(BOOL)shouldPerformSegueWithIdentifier:(NSString *)identifier sender:(id)sender{
    ...
    if ([identifier isEqual:@"addTransfer"]){
        if (([self.tableViewOut indexPathForSelectedRow]!=nil) &
            ([self.tableViewIn indexPathForSelectedRow]!=nil))
            return YES;
    }
    return NO;
